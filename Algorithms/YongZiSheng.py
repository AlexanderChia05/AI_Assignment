...
def visualize_maze(maze, start, end, current_path, traversed_nodes, step, ansi=True):
    ...
    if step == 1:
        ...
        delay = 0.5
        explanation = "Backtracking: Returning to explore other branches..."
    else:
        explanation = "Exploring: Moving deeper into the maze..."
...
def dfs(start, end, maze, visualize=False, ansi=True):
    """
    ...
    while frontier:
        current, path = frontier.pop()  # Pop the last element (LIFO)
...
def main():
        ...
        while True:
            ...
            try:
                ...
                if 1 <= choice <= len(maze_test_cases):
                    ...
                    dfs(test_case["start"], test_case["end"], test_case["maze"], visualize=True, ansi=ansi_color)
                else:
    ...
    for i, test_case in enumerate(maze_test_cases, 1):
        ...
        path, nodes_expanded, time_taken, traversed_nodes, peak_memory = dfs(start_point, end_point, maze, visualize=False)
    ...
    RED = "\033[1;31m"
    RESET = "\033[0m"
    print("\n\nAlgorithm Performance Metrics for DFS")
    print("+--------+---------------------+-------+-------------------+-------------------+-------------------+-------+-------------------+")
