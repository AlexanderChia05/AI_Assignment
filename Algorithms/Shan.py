...
def visualize_maze(maze, start, end, current_path, traversed_nodes, step, ansi=True):
    ...
    if step == 1:
        ...
        delay = 0.5
        explanation = "Backtracking: Exploring a different path..."
    else:
        explanation = "Exploring: Moving to next position in queue..."
...
def main():
        ...
        while True:
            ...
            try:
                ...
                if 1 <= choice <= len(maze_test_cases):
                    ...
                    bfs(test_case["start"], test_case["end"], test_case["maze"], visualize=True, ansi=ansi_color)
                else:
    ...
    for i, test_case in enumerate(maze_test_cases, 1):
        ...
        path, nodes_expanded, time_taken, traversed_nodes, peak_memory = bfs(start_point, end_point, maze, visualize=False)
    ...
    RED = "\033[1;31m"
    RESET = "\033[0m"
    print("\n\nAlgorithm Performance Metrics for BFS")
    print("+--------+---------------------+-------+-------------------+-------------------+-------------------+-------+-------------------+")
