...
def visualize_maze(maze, start, end, current_path, traversed_nodes, step, temperature, ansi=True):
    """
    ...
    delay = 0.1
    if step == 1:
        explanation = "Starting simulated annealing from initial position..."
    elif current_position == end:
        explanation = "Goal reached! Path found."
    elif backtracking:
        delay = 0.5
        explanation = "Backtracking: Accepted a worse solution to escape local optima..."
    else:
        explanation = "Exploring: Moving to a random neighbor..."
...
def simulated_annealing(start, end, maze, visualize=False, ansi=True):
    """
    Solve the maze using Simulated Annealing Search.
    ...
    current_path = [start]
    current_cost = heuristic(start, end)
    ...
    while temperature > min_temp and iteration < max_iterations:
        ...
        next_cost = heuristic(next_position, end)

        # Calculate cost difference
        cost_diff = next_cost - current_cost

        # Accept the move based on cost difference and temperature
        if cost_diff <= 0 or random.random() < math.exp(-cost_diff / temperature):
            current_position = next_position
            current_path.append(next_position)
            current_cost = next_cost
            visited.add(next_position)
            traversed_nodes.append(next_position)
        ...
        if visualize:
            visualize_maze(maze, start, end, current_path, traversed_nodes, step, temperature, ansi=ansi)
    ...
    path = best_path if best_position == end else []
    nodes_traversed = len(traversed_nodes)

    if visualize:
        visualize_maze(maze, start, end, path, traversed_nodes, step + 1, temperature, ansi=ansi)
        if path:
...
def main():
        ...
        while True:
            ...
            try:
                ...
                if 1 <= choice <= len(maze_test_cases):
                    ...
                    simulated_annealing(test_case["start"], test_case["end"], test_case["maze"], visualize=True, ansi=ansi_color)
                else:
    ...
    for i, test_case in enumerate(maze_test_cases, 1):
        ...
        path, nodes_expanded, time_taken, traversed_nodes, peak_memory = simulated_annealing(start_point, end_point, maze, visualize=False)
    ...
    RED = "\033[1;31m"
    RESET = "\033[0m"
    print("\n\nAlgorithm Performance Metrics for SIMULATED ANNEALING")
    print("+--------+---------------------+-------+-------------------+-------------------+-------------------+-------+-------------------+")
